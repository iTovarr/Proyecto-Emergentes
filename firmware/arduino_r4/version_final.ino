...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (String(topic) == "Sistema/Puerta" && mensaje == "Puerta_Abierta") {
    ...
    servoMotor.write(180);  // regresar a 0°
    Serial.println("🚪 Puerta cerrada");
  }

  // --- Control de techo (relay) ---
  if (String(topic) == "Sistema/Humedad" && mensaje == "Techo_Activado") {
    digitalWrite(relayPin, HIGH);   // activar relay
    delay(3000);                   // mantener 3 segundos
    digitalWrite(relayPin, LOW);    // desactivar relay
  }
}
...
void setup() {
  ...
  digitalWrite(buzzerPin, LOW);   // buzzer apagado por defecto
  digitalWrite(relayPin, HIGH);   // relay apagado por defecto

  servoMotor.attach(5);
  servoMotor.write(180); // servo inicial en 0°
